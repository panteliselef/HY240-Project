...
/**
 * This function prints the horser of the soldiers of GA
 ...
 */
void Add2GA_Battle(struct GA_Battle **first,struct GA_Battle **last,
...
/**
 * This function call Add2GA_Battle with different horse id
 * depending on the type of the soldier
 * @param first   		The first node of the GA_battle_list
 * @param last    		The last node of the GA_battle_list
 * @param soldier     The current struct Army node from GA_Army[0] (solidier)
 * @param soldier     The current struct Army node from GA_Army[1] (horse)
 * @param type     		An indicator of what type of soldier should be inserted
 */
struct GA_Battle* fillList(struct GA_Battle **first ,struct  GA_Battle **last,
														struct Army *soldier,struct Army *horse, int type) {

		while(soldier!=NULL) {
			if(soldier->type == type) {
				if(soldier->type ==1 && horse != NULL) {
					Add2GA_Battle(first,last,soldier->id,soldier->general,horse->id);
					soldier = soldier->next;
					horse = horse->next;
				}
				else if(soldier->type ==1 && horse == NULL) {
					Add2GA_Battle(first,last,soldier->id,soldier->general,-2);
					soldier = soldier ->next;
				}
				else {
					Add2GA_Battle(first,last,soldier->id,soldier->general,-1);
					soldier = soldier ->next;
				}
			}else{
				soldier = soldier ->next;
...
int prepare_for_battle_GA (void)
{
	...
	GA_battle_list = fillList(&GA_battle_list,&GA_battle_end,GA_army[0],GA_army[1],1);
	GA_battle_list = fillList(&GA_battle_list,&GA_battle_end,GA_army[0],GA_army[1],2);
	GA_battle_list = fillList(&GA_battle_list,&GA_battle_end,GA_army[0],GA_army[1],3);
	GA_battle_list = fillList(&GA_battle_list,&GA_battle_end,GA_army[0],GA_army[1],4);
 ...
 */
void Add2BattleS(struct S_Battle **first,struct S_Battle **last,
...
int prepare_for_battle_S (int sid)
{
	...
	while(q!=s->sentinel) {
		...
		Add2BattleS(&first,&last,s->id,q->id,q->type);
		q = q->next;
	}
